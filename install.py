...
def update_transparent_background():
    ...
    if version.parse(v) < version.parse('1.2.3'):
        launch.run_pip("install -U transparent-background", "update transparent-background version for Face-Aging-1111")
...
if platform.system() == "Darwin" and platform.machine() == "arm64":
    if not launch.is_installed("pyvirtualcam"):
        launch.run_pip("install git+https://github.com/letmaik/pyvirtualcam", "requirements for Face-Aging-1111")

if not launch.is_installed("transparent_background"):
    launch.run_pip("install transparent-background", "requirements for Face-Aging-1111")

update_transparent_background()

if not launch.is_installed("IPython"):
    launch.run_pip("install ipython", "requirements for Face-Aging-1111")

if not launch.is_installed("seaborn"):
    launch.run_pip("install ""seaborn>=0.11.0""", "requirements for Face-Aging-1111")

if not launch.is_installed("color_matcher"):
    launch.run_pip("install color-matcher", "requirements for Face-Aging-1111")
