    ...
    with gr.Blocks(analytics_enabled=False) as face_aging_interface:
        with gr.Row().style(equal_height=False):
            with gr.Column(variant='panel'):
                ...
                master_folder_preprocess = gr.Textbox(label="Master Folder Path (for preprocessing)", name="master_folder_preprocess")
                gr.Button(label="Execute Preprocessing", action=preprocess_video)
                
                # GUI for split_aged_tiled_image_square.py
                master_folder_split = gr.Textbox(label="Master Folder Path (for split image)", name="master_folder_split")
